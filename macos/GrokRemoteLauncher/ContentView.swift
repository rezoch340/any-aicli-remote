...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .onChange(of: settings.publicHost) { _, _ in controller.rebuildFallbackPairing() }
        .onChange(of: settings.daemonPort) { _, _ in controller.rebuildFallbackPairing() }
        .onChange(of: settings.workspace) { _, _ in controller.rebuildFallbackPairing() }
    }
    ...
    private var header: some View {
        HStack(spacing: 14) {
            ...
            Button(action: controller.performPrimaryAction) {
                Label(controller.showsStopAction ? "停止服务" : "启动服务",
        ...
        Card(title: "服务配置", systemImage: "slider.horizontal.3") {
            VStack(alignment: .leading, spacing: 14) {
                VStack(alignment: .leading, spacing: 7) {
                    ...
                    HStack {
                        TextField("选择工作区", text: $settings.workspace)
                            .textFieldStyle(.roundedBorder)
                        Button("选择…", action: controller.chooseWorkspace)
                    }
                }

                HStack(spacing: 14) {
                    Field(title: "Daemon 端口") {
                        TextField("2421", value: $settings.daemonPort, format: .number.grouping(.never))
                            .textFieldStyle(.roundedBorder)
                    }
                    Field(title: "Agent 端口") {
                        TextField("2419", value: $settings.agentPort, format: .number.grouping(.never))
                            .textFieldStyle(.roundedBorder)
                    }
                    Field(title: "Bind 地址") {
                        TextField("0.0.0.0", text: $settings.bindAddress)
                            .textFieldStyle(.roundedBorder)
                    }
                }

                Field(title: "公网地址（可选）") {
                    TextField("https://remote.example.com", text: $settings.publicHost)
                        .textFieldStyle(.roundedBorder)
                }

                Divider()
                HStack(alignment: .firstTextBaseline, spacing: 9) {
                    Image(systemName: controller.daemonExecutablePath.isEmpty ? "exclamationmark.triangle.fill" : "checkmark.seal.fill")
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text(controller.daemonExecutablePath.isEmpty ? "未找到，请先构建后端" : controller.daemonExecutablePath)
                            .font(.system(.caption, design: .monospaced))
                            .lineLimit(2)
                            .textSelection(.enabled)
                    }
                    Spacer()
                    Button { controller.refreshDaemonLocation() } label: {
        ...
        Card(title: "手机配对", systemImage: "qrcode") {
            VStack(spacing: 12) {
                ...
                HStack {
                    ...
                    Button { controller.openPairingURL() } label: { Image(systemName: "safari") }
                        .help("在浏览器中打开")
                }
            ...
            VStack(spacing: 8) {
                HStack(spacing: 12) {
                    if let health = controller.health {
                        Label(health.hubUp == true ? "Hub 已连接" : "Hub 等待中",
...
private struct Card<Content: View>: View {
    ...
    var body: some View {
        ...
        .padding(18)
        .background(Color(nsColor: .controlBackgroundColor).opacity(0.92), in: RoundedRectangle(cornerRadius: 17, style: .continuous))
