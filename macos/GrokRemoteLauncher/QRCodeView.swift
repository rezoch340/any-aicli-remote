...
struct QRCodeView: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel("Grok Remote 配对二维码")
    }
}
...
private enum QRCodeRenderer {
    static func image(for payload: String) -> NSImage? {
        ...
        let background = CIImage(color: .white).cropped(to: CGRect(x: 0, y: 0, width: side, height: side))
        let composed = translated.composited(over: background)
        guard let cgImage = CIContext(options: [.useSoftwareRenderer: false])
            .createCGImage(composed, from: composed.extent) else { return nil }
