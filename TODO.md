...
- [ ] **8. 扩展流式与交互 primitive（全部归一化）**
  ...
  - [~] Phase 4（feedback 交互）：复核开源后判定**不做**。`feedback_request` 实为 xAI 的评分/NPS 产品反馈漏斗——`ClientFeedbackInput` 带 `rating_type`（thumbs/stars/nps）、thumbs 值 -1/0/1，挂在 `x.ai/rollout/survey` / engagement tier 上，dismissible，与"操控远程 agent"无关。当前客户端静默忽略即正确行为；铺"通知进 + `x.ai/feedback` 评分调用出 + 中立方法 + 两端 thumbs/NPS UI"是最高成本换最低价值（把打分漏斗焊到控制类 app）。YAGNI，保留静默忽略。
