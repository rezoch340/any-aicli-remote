    ...
    @MainActor final class Coordinator: NSObject, UICollectionViewDelegate {
        ...
        private func updateFollowingScrollPosition(startingOffset: CGPoint) {
            ...
            if let clampDeadline = sessionLoadClampDeadline {
                ...
                pinToBottom()
            } else if !animateStreamingFollow(startingOffset: startingOffset) {
                pinToBottom()
            }
        }

        private func completeInitialRevealIfReady(collection: UICollectionView) -> Bool {
        ...
        private func performPendingUpdate() {
            guard needsFinalFlush else { return }
            needsFinalFlush = false
            let flushGeneration = sessionGeneration
            let flushIdentity = identity
            DispatchQueue.main.async { [weak self] in
                guard let self, self.sessionGeneration == flushGeneration,
                      self.identity == flushIdentity,
        ...
        private func animateStreamingFollow(startingOffset: CGPoint) -> Bool {
            ...
            UIView.animate(withDuration: Metrics.streamingAnimationDuration,
                ...
                let targetOffset = CGPoint(x: collection.contentOffset.x, y: maximumOffset)
                collection.setContentOffset(targetOffset, animated: false)
            }
            return true
        }
