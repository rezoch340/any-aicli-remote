...
final class PendingUserEchoTracker {
  ...
  func consume(chunk: String) -> Bool {
    ...
    let advance: Int
    if chunk == expected {
      advance = expectedCharacters.count - consumed
    } else if remaining.hasPrefix(chunk) {
      advance = chunkCharacters.count
    } else if expected.hasPrefix(chunk) {
      advance = chunkCharacters.count - consumed
    } else {
      advance = overlapAdvance(expectedCharacters, chunkCharacters, consumed: consumed)
    }
    ...
    if consumedLength == expectedCharacters.count {
      clear()
    }
    return true
  }
  ...
  private func overlapAdvance(_ expected: [Character], _ chunk: [Character], consumed: Int) -> Int {
    ...
    for overlap in stride(from: min(consumedCharacters.count, chunk.count), through: 1, by: -1) {
      if Array(consumedCharacters.suffix(overlap)) == Array(chunk.prefix(overlap)),
        Array(expected.dropFirst(consumed).prefix(chunk.count - overlap))
          == Array(chunk.dropFirst(overlap)) {
