...
public struct DeviceHealthProbe: Sendable {
    private let session: URLSession
    private let runtimeConfiguration: ClientRuntimeConfiguration

    public init(session: URLSession = .shared, runtimeConfiguration: ClientRuntimeConfiguration = ClientRuntimeConfiguration()) {
        self.session = session
        self.runtimeConfiguration = runtimeConfiguration
    }

    public func isOnline(baseURL: URL) async -> Bool {
