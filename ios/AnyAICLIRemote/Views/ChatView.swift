    ...
    private var chatContent: some View {
        ...
        .safeAreaInset(edge: .bottom, spacing: 0) {
            VStack(spacing: 0) {
                ...
                ComposerView(text: $draft, isBusy: store.isBusy, status: store.statusMessage, attachments: store.selectedFiles, onAttach: { store.browseWorkspace() }, onRemove: { store.removeFile($0) }) {
                    let outgoing = draft
                    draft = ""
                    shouldFollow = true
                    scrollRequestRevision += 1
