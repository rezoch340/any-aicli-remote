...
struct ChatBlockView: View {
    ...
    var body: some View {
        ...
        case .assistant: AssistantMessage(block: block)
        case .thinking: ThinkingBlock(block: block)
        case .tool: ToolCapsule(block: block)
        case .permission: PermissionCard(block: block)
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                ForEach(block.options) { option in
                    Button(option.label) { store.answerPermission(blockID: block.id, optionID: option.id) }
                        .buttonStyle(.borderedProminent)
                        .tint(option.id.lowercased().contains("reject") ? .red : .cyan)
                }
                Button("取消", role: .cancel) { store.answerPermission(blockID: block.id, optionID: nil) }
